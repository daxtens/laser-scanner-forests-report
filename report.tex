...
\subsubsection{Tree Detector: \texttt{tree\_detector}}
...
detected---is shown in Figure~\ref{fig:tennis-results}. \todo{Video
  and data bag location}
...
\begin{figure}
  ...
  \caption{RSISE flight results, showing a snapshot of the
    flight. \todo{replot with corrected height}}
...
Waiting to extract the results

\chapter{Discussion}
\label{cha:discussion}

\todo{some sort of introduction} In many cases, each subsection is
...
data, but that was easy to address (see \todo{discussion of it}).
...
The process
is further complicated because the algorithm cannot be sure that all
the points that have just been detected map to points on the map: some
--- or all! --- of them could be new detections.

Forests tend to look similar at small scales. Just like a corner is
not a particuarly unique feature of a building, a pair of trees is
often not very unique. They tend to grow to be the same distance
apart!

This could be assisted with radii, but perfect column assumption.
...
approach taken by \texttt{slam\_gmapping} \todo{link, explanation}

Extend to 3D with use of voxels; at risk of reducing the largeish
efficiency gains.
...
\subsection{Software: future work}
...
% This is sort of inherent.
%\subsection{Hardware: future work}
%\label{sec:hardw-future-work}
...
also other potential weight savings that could be realised by a
...
weight is to balance the hexacopter as a whole. \todo{explain}
...
the second flight trial.
 ...
 \end{figure}
    ...
    - put a frame around the top too.
    - see also the issues identified with the base plate anyway.
