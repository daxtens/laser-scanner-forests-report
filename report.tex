...
\newacronym{PSM}{PSM}{Polar Scan Matching}
\newacronym{ICP}{ICP}{Iterated Closest Point}
...
%problems (what motivates the study); 
...
%state the method of investigation (the 
%main technical issues, including important limitations in scope);
...
\item If the common set only contains one point, then a simple
  translation is calculated, and it is assumed there is no rotation. \todo{why}
\item If there are no points in common, the transform is assumed to be
  the identity: no translation or rotation. \todo{why}
...
utilising something like a Kalman filter. \todo{cite}
...
\todo{cite}.) This should be addressed.
...
%Your conclusion should not restate the results; instead, it should refer back 
%to the ‘real world’ problem that motivated the study in the first place. 
%What questions associated with that problem are you now able to answer? 
%You may also make recommendations for further study
